...
- **PM decisions executed (2026-08-25)**: #53 theming shipped (`feat/theme-system`, PR #62): `shellframe_theme_load` + default/mono themes, 13 unit assertions. #55 ADR 0001 accepted (both APIs kept; v1 = standalone on-ramp, internals converge opportunistically, new surfaces v2-only). #58 POSIX docs + msys/cygwin load warning. #57 split per worker recommendation: current state documented, motion support re-scoped and tracked in #57 (implementation ticket, open) — feasibility improved by #51 throttling. #56 pager approved by PM, **not yet started** — next implementation item. #54 stays open for contributors.
