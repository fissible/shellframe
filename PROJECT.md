...
- **diff-view render coverage (2026-03-23)**: Added 23 new assertions to `test-diff-view.sh` covering `_shellframe_dv_clip_ansi`, `_shellframe_dv_render_pane` (all 7 row types: hdr, file_sep, ctx, add, del, chg, sep; blank-opposite-side logic; HIDE_FILE_HDR path), `shellframe_diff_view_render` (with/without footer), and `shellframe_diff_view_render_side`. Uses `_dv_capture` helper (fd 3 → tmpfile → ANSI-strip). `widgets/diff-view.sh` coverage: **11% → 76%**. Total unit assertions: 880/880. Overall coverage: **64%** (up from 58%). HTML report at `coverage/2026_03_23_05_19_32.html` (added to index).
