...
- **#45–#47 hardening session (2026-08-25)**: Three M-severity review items closed on branch `fix/45-46-47-hardening` (PR pending at write time).
  - [#45](https://github.com/fissible/shellframe/issues/45): `shellframe_sanitize` (clip.sh) strips ANSI + C0 from untrusted text; `shellframe_read_key` gains optional timeout → `SHELLFRAME_KEY_TIMEOUT`; editor paste drain bounded by `SHELLFRAME_PASTE_SILENCE_LIMIT` (default 5s) and sanitized. PTY-verified lost-terminator recovery.
  - [#46](https://github.com/fissible/shellframe/issues/46): SGR decoding consolidated into `_shellframe_parse_sgr_mouse`; malformed events discarded; motion (bit 32) flagged via `SHELLFRAME_MOUSE_MOTION` and dropped (#57 owns hover).
  - [#47](https://github.com/fissible/shellframe/issues/47): single-column width limitation documented in README + api.md; wcwidth stays proposed under #54.
  - Suite **1555/1555 across 51 files**; Docker matrix **3/3 PASS** locally — the long-standing "matrix blocked by file-sharing" note is fixed: run-matrix now prefers a sibling ptyunit checkout under `/Users` over the unshared `/opt/homebrew` mount.
  - **Gotchas worth keeping**: `$'..'` escapes do NOT expand inside case-pattern bracket expressions (build classes with printf); an unquoted `=~` regex containing `;` is a parse error (quote into a variable); pty_run `--expect` checks once immediately after the prior key settles — it cannot poll, so don't use it as a "wait for" primitive.
  - **Next**: reviewer sign-off on the PR, then remaining M/L items (#48–#52) and PM-gated decisions (#54, #55, #57).
- **#48–#52 hardening batch (2026-08-25)**: All five remaining review issues closed on branch `fix/48-52-batch` (PR open).
  - [#50](https://github.com/fissible/shellframe/issues/50): premise not reproducible — `a[$i]+="x"` under `-u` is an assignment (safe on 3.2/5.x, verified empirically); reads were already guarded. Sparse-row + frame-growth regression tests added.
  - [#49](https://github.com/fissible/shellframe/issues/49): the `.toolrc.local` cache **never existed in any revision** (`git log --all -S` archaeology) — README/CLAUDE.md described phantom machinery. Docs corrected to describe inline capability checks; PM flag: agent-guidance doc drift.
  - [#48](https://github.com/fissible/shellframe/issues/48): `SHELLFRAME_TTY_FD` knob (default 3); all 43 fd writes route through it. Deviation: no 4.1+ auto-allocation — one predictable number per session instead.
  - [#51](https://github.com/fissible/shellframe/issues/51): coalescing root-caused — the old `read -t 0` probe **consumes buffered input on bash ≥4** ("XYZ" vanished in repro) and is inert on 3.2; lesson #10 recorded. Re-enabled TIME-based (33ms window / 200ms force cap / no-clock fallback). Busybox gotcha found en route: `date +%s%3N` exits 0 printing seconds-only — validate output format, not exit status.
  - [#52](https://github.com/fissible/shellframe/issues/52): spinner/progress/status widgets shipped (`src/widgets/spinner.sh`), non-tty degradation + exit-code contract honored; v2-region status variant deferred to pair with #53 theming.
  - Suite **1586/1586 across 53 files**; Docker matrix **3/3 PASS**.
- **v0.6.0 released (2026-08-25)**: Review round 3/4 fixes merged via #61 (save-slot fd choreography repaired with a working openness probe, spinner capture, corrected lesson #10, macOS-leg timing). Phase 8: **12/18 closed (#41–#52)**. Remaining #53–#58 are enhancements/PM-gated decisions (theming, wcwidth strategy, v1/v2 ADR, pager escape hatch, hover scope).
- **PM decisions executed (2026-08-25)**: #53 theming shipped (`feat/theme-system`, PR #62): `shellframe_theme_load` + default/mono themes, 13 unit assertions. #55 ADR 0001 accepted (both APIs kept; v1 = standalone on-ramp, internals converge opportunistically, new surfaces v2-only). #58 POSIX docs + msys/cygwin load warning. #57 split per worker recommendation: current state documented, motion support (1002 drag / 1003 hover behind opt-in) filed as open M-effort contributor issue — feasibility improved by #51 throttling. #56 pager approved by PM, **not yet started** — next implementation item. #54 stays open for contributors.
